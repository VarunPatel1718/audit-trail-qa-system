...
**Current phase:** Phase 8 — RAG Layer (policy retrieval complete; similar-case retrieval outstanding)
...
1. Phase 8 — Similar case retrieval (backed by the existing `audit_cases` table) — the one remaining unchecked Phase 8 item.
2. Phase 9 — MCP Server: wrap the ledger/rule-engine/RAG services already built (query_ledger, flag_discrepancy, generate_audit_note, retrieve_policy, retrieve_similar_cases, risk_score, export_report) as MCP tools.
