...
**Current phase:** Phase 7 — Rule Engine (complete)
...
1. Phase 8 — RAG Layer: policy PDF chunking, embeddings (sentence-transformers), vector DB setup (Qdrant/ChromaDB)
2. Phase 8 — policy retrieval and similar-case retrieval, backed by the `audit_cases`/`policies` tables already in the schema
3. Longer-term (not blocking): revisit `app/services/risk_scoring.py`'s risk-level score bands now that all 9 per-transaction rules can stack (max 185 points) — still monotonic and behaving sensibly per this session's verification, but worth a deliberate pass once real usage patterns (not just synthetic seed data) are available
