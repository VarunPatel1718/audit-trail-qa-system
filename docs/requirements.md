...
6. **AI-Generated Audit Notes** — Given a flagged transaction, the system uses an LLM to draft a structured, explainable audit note grounded in the retrieved policies and cases. **Deviation from docs/design.docx:** design.docx specifies the Claude API; the implementation uses Groq (Llama 3.3 70B) instead, for cost reasons (Groq's free tier avoids paid API credits). Revisit if quality/accuracy requirements call for Claude's stronger reasoning — see PROGRESS.md decisions log.
